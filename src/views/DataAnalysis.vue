<template>
    <div class="main-layout">
      <div class="main-content">
        <div v-if="getDataInstances.length > 0">
  ...
  import { OneSampleTTest, IndependentTwoSampleTTest, PairedSampleTTest, KruskalWallisTest, MannWhitneyUTest, OneWayANOVA } from '../classes/StatisticalModel';
  ...
  export default {
    ...
    data() {
      return {
        ...
        stats: [
          [new OneSampleTTest(), new IndependentTwoSampleTTest(), new PairedSampleTTest(), new KruskalWallisTest(), new MannWhitneyUTest(), new OneWayANOVA()]
        ],
      };
    },
    computed: {
    ...
    methods: {
      ...
      addNewStatCard() {
        this.stats.push([new OneSampleTTest(), new IndependentTwoSampleTTest(), new PairedSampleTTest(), new OneWayANOVA()]);
      },
      ...
      async handleSubmitStat(statConfig) {
        ...
        const dataToSend = {
          data: this.getDataInstances[this.activeInstanceIndex].data.map(row => ({ columns: row }))
        };
        const requestBody = {
          data: dataToSend,
