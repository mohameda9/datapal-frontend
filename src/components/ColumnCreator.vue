<template>
  <div class="AdvancedColumnCreator-backdrop" role="dialog" aria-labelledby="column-modal-title" aria-describedby="column-modal-desc">
    <div class="AdvancedColumnCreator">
      ...
      <section class="AdvancedColumnCreator-body" id="column-modal-desc">
        <slot name="body">
          <p>Column name:</p>
          <input v-model="columnName" @input="CheckNewColumnName($event.target.value)" aria-describedby="column-error-message" />
          <p v-if = "columnNameExistsWarn !=null" :style="{color: 'red'}">{{ columnNameExistsWarn }}</p>

          <div v-for="(conditionGroup, groupIndex) in conditionGroups" :key="groupIndex" class="condition-group">
            <p>Condition Group {{ groupIndex + 1 }}:</p>
            <div v-for="(condition, condIndex) in conditionGroup.conditions" :key="condIndex" class="condition">
              <select v-model="condition.column" class="column-select" @change="updateOperators(groupIndex, condIndex)">
                <option v-for="col in availableColumns" :key="col" :value="col">{{ col }}</option>
              </select>
              <select v-model="condition.operator">
                <option v-for="operator in condition.availableOperators" :key="operator" :value="operator">{{ operator }}</option>
              </select>
              <input v-model="condition.value" placeholder="Value" />
              <button type="button" class="btn-remove" @click="removeANDCondition(groupIndex, condIndex)" aria-label="Remove condition">Remove</button>
            </div>
            <div class="result-row">
              <input v-model="conditionGroup.result" :placeholder="conditionGroup.resultType === 'expression' ? 'Result Expression' : 'Result Value'" class="result-input"/>
              <select v-model="conditionGroup.resultType" class="result-type-select">
                <option value="value">Value</option>
                <option value="expression">Expression</option>
              </select>
            </div>
            <div v-if="conditionGroup.errorMessage" class="error-message">{{ conditionGroup.errorMessage }}</div>
            <button type="button" class="btn-add" @click="addANDCondition(groupIndex)">Add an ADD clause</button>
          </div>
          <button type="button" class="btn-and" @click="addConditionGroup">Add condition group</button>

          <p>Default (else) value:</p>
          <div class="result-row">
            <input v-model="defaultValue" :placeholder="defaultValue === 'expression' ? 'Default Expression' : 'Default Value'" class="result-input"/>
            <select v-model="defaultValueType" class="result-type-select">
              <option value="value">Value</option>
              <option value="expression">Expression</option>
            </select>
          </div>
          <div v-if="elseErrorMessage" class="error-message">{{ elseErrorMessage }}</div>

          <div v-if="errorMessage" class="error-message" id="column-error-message">{{ errorMessage }}</div>
        </slot>
      </section>
...
methods: {
  ...
  submit() {
    ...
    this.conditionGroups.forEach(group => {
      if (group.resultType === "expression") {
    ...
    if (this.defaultValueType === "expression") {
    ...
    const metaData = this.conditionGroups.map(group => ({
      conditions: group.conditions.map(cond => ({
        ...
        value: cond.operator === "isin" ? cond.value.split(',').map(val => val.trim()) : cond.value
      })),
      result: group.result,
  ...
  validateExpression(expression) {
    ...
    if (!this.isValidExpression(expression)) {
      expressionValidationMessage = 'Expression syntax is invalid. Please check for balanced parentheses, logical syntax, and ensure all identifiers are correctly referenced as model.columnname.';
    ...
    if (invalidColumns.length > 0) {
      expressionValidationMessage = `Invalid columns: ${invalidColumns.join(', ')}, columns must be numeric or numeric binary in expressions`;
      return [false, expressionValidationMessage];
    } else {
  ...
  columnExists(columnName) {
    let numericalColumns = Object.keys(this.columnTypes).filter(key => ["numeric", "numeric binary"].includes(this.columnTypes[key]));
  
    return numericalColumns.includes(columnName);
  },
  ...
  CheckNewColumnName( newColumnName){
      ...
      if (this.availableColumns.some(column => column == newColumnName)){
          this.columnNameExistsWarn = "column name already exists; will be overwriten if you continue"
      }
      else{
